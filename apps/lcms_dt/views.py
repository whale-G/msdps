...
from .tasks import process_lcms_ab_files, process_lcms_agilent_files
        ...
        try:
            ...
            for file_obj in uploaded_files:
                if not file_obj.name.lower().endswith('.docx'):
                    ...
                    continue
                # 读取文件内容并转为Base64编码
                file_content = file_obj.read()  # 读取为字节
                file_content_base64 = base64.b64encode(file_content).decode('utf-8')    # 转为Base64字符串
            ...
            # 启动异步任务，任务信息存入Redis
            try:
                task = process_lcms_ab_files.delay(file_contents, str(uuid), process_id)
...
class LcmsProcessAbStatus(APIView):
    """查询AB液质处理任务状态的视图"""
...
class LcmsProcessAgilent(APIView):
    """安捷伦液质数据处理视图"""
    parser_classes = [MultiPartParser, FormParser]
    ...
    def post(self, request):
        ...
        try:
            ...
            # 启动异步任务，任务信息存入Redis
            try:
                task = process_lcms_agilent_files.delay(file_contents, str(uuid), process_id)
            except Exception as e:
                return Response({
                    "status": "error",
                    "message": "创建异步任务失败",
...
class LcmsProcessAgilentStatus(APIView):
    """查询安捷伦液质处理任务状态的视图"""
