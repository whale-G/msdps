...
from .tasks import process_gcms_shimazu_files, process_gcms_thermo_file
...
class GcmsProcessShimazu(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # 启动异步任务，任务信息存入Redis
            try:
                task = process_gcms_shimazu_files.delay(file_contents, str(uuid), process_id, float_parameter)
...
class GcmsProcessShimazuStatus(APIView):
    """查询岛津气质处理任务状态的视图"""
...
class GcmsProcessThermo(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # 启动异步任务，任务信息存入Redis
            try:
                task = process_gcms_thermo_file.delay(file_contents, str(uuid), process_id, float_parameter)
            except Exception as e:
                return Response({
                    "status": "error",
                    "message": "创建异步任务失败",
...
class GcmsProceeThermoStatus(APIView):
    """查询热电气质处理任务状态的视图"""
