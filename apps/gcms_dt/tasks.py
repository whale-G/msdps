...
def process_gcms_shimazu_files(self, file_contents_list, user_id, process_id, float_parameter):
    """
    异步处理GCMS_shimazu文件的Celery任务，在后台进行
    ...
    try:
        ...
        for idx in range(1, len(processed_results)):
            ...
            for data in processed_results[idx]["data"][1:]:
                ...
                for base_idx in range(len(base_temp)):
                    ...
                    if (base_temp[base_idx][0] + float_parameter) >= data[0] >= (
                            base_temp[base_idx][0] - float_parameter) and base_temp[base_idx][1] == data[1]:
        ...
        # 将处理结果保存至数据库
        Gcms_UserFiles.objects.create(
            process_id=process_id,
            user=user,
            file_type="gcms-shimazu-2010&8050",
...
@shared_task(bind=True)
def process_gcms_thermo_file(self, file_contents_list, user_id, process_id, float_parameter):
    """
